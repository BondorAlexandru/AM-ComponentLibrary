...
function Field({ children, error, className = '' }) {
    return (_jsx("div", { className: `space-y-1 ${className}`, "data-field-error": error ? 'true' : undefined, children: children }));
}
function Label({ children, htmlFor, required, action, className = '' }) {
    return (_jsxs("div", { className: "flex items-center justify-between", children: [_jsxs("label", { htmlFor: htmlFor, className: `font-display uppercase text-[12px] font-normal tracking-[1.2px] text-ink-3 ${className}`, children: [children, required && _jsx("span", { className: "text-danger-accent ml-1", children: "*" })] }), action && _jsx("div", { className: "text-xs", children: action })] }));
}
const TextInput = forwardRef(({ error, size = 'md', className = '', ...props }, ref) => {
    ...
    return (_jsx("input", { ref: ref, className: `
          w-full rounded-[8px]
          ${sizeClasses[size]}
          ${errorClasses}
...
const Textarea = forwardRef(({ error, resize = 'vertical', className = '', ...props }, ref) => {
    ...
    return (_jsx("textarea", { ref: ref, className: `
...
const Select = ({ error, size = 'md', children, className = '', value, onChange, disabled, ...props }) => {
    ...
    return (_jsx(Dropdown, { value: value === undefined || value === null ? undefined : String(value), onChange: (e) => onChange?.({ target: { value: e.target.value } }), disabled: disabled, "aria-label": props['aria-label'], title: props.title, className: `
        w-full rounded-[8px]
        ${sizeClasses[size]}
        ${errorClasses}
        border bg-input text-ink
...
const Checkbox = forwardRef(({ label, error, className = '', ...props }, ref) => {
    ...
    return (_jsxs("div", { className: "flex items-center gap-2", children: [_jsx("input", { ref: ref, type: "checkbox", id: id, className: `
            h-4 w-4 rounded
            ${errorClasses}
            border-2
            transition-colors
            ...
            ${className}
          `, ...props }), label && (_jsx("label", { htmlFor: id, className: "text-sm text-ink-2 cursor-pointer", children: label }))] }));
});
...
const Radio = forwardRef(({ label, error, className = '', ...props }, ref) => {
    ...
    const errorClasses = error
        ? 'border-danger-accent text-danger-accent focus:ring-danger-accent'
        : 'border-line text-accent focus:ring-accent';
    return (_jsxs("div", { className: "flex items-center gap-2", children: [_jsx("input", { ref: ref, type: "radio", id: id, className: `
            h-4 w-4
            ${errorClasses}
            border-2
            transition-colors
